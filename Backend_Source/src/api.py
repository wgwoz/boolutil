...
# JEDYNA, właściwa inicjalizacja aplikacji
app = FastAPI()

# Prawidłowa konfiguracja CORS (teraz będzie działać!)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"], 
...
class ExpressionIn(BaseModel):
    ...
    ttable_readable: Optional[Tuple[List[int], List[int]]] = None  # Poprawione pod wysyłany format
...
class ExpressionOut(BaseModel):
    ...
    sympy_expr: str
    vars: List[str]
    ttable_readable: Optional[Tuple[List[int], List[int]]] = None
...
def create_expression(payload: ExpressionIn):
    try:
        if payload.plaintext is not None:
            ...
            # Konwersja formatu tabeli dla biblioteki, jeśli wymagana
...
@app.options("/expression/nand")
def options_expression_nand(response: Response):
    response.headers["Access-Control-Allow-Origin"] = "*"
    response.headers["Access-Control-Allow-Methods"] = "POST, OPTIONS"
    response.headers["Access-Control-Allow-Headers"] = "*"
    return response
